...
- The important domain logic (OHID resolution, repository contracts, event publishing,
  WhatsApp and Twilio webhook verification) can be imported and tested in constrained
  environments that do not provide `ssl` or optional email validation packages.
...
class LeadIngestRequest(BaseModel):
    ...
    source_system: str = Field(..., pattern=r"^(META|WEB|WHATSAPP|TWILIO|ZOHO_SOCIAL|ZOHO_CRM)$")
    source_lead_id: str
    channel: str = Field(
        ...,
        pattern=r"^(WEB_FORM|META_LEAD_AD|INBOUND_CALL|OUTBOUND_CALL|WHATSAPP|SMS|SOCIAL|CRM)$",
    )
    person: Person
...
class TwilioWebhookPayload(BaseModel):
    """Payload from Twilio webhook (SMS or Voice status callback)."""
...
class WhatsAppMessage(BaseModel):
    """A single message from the WhatsApp Cloud API webhook."""
...
if FastAPI is not None:  # pragma: no cover - exercised in real runtime only
    ...
    def verify_twilio_signature(url: str, params: Dict[str, str], signature: str) -> bool:
        ...
        concatenated as key=value pairs, then base64-encoded.
        """
        import base64
    ...
    async def twilio_webhook(
        ...
        payload = TwilioWebhookPayload(**params)
    ...
    async def whatsapp_webhook(
        request: Request,
        x_hub_signature: str = Header(default="", alias="X-Hub-Signature-256"),
        repo: Repository = Depends(repo_dep),
    ):
        raw_body = await request.body()
        if not verify_whatsapp_signature(raw_body, x_hub_signature):
        ...
        for entry in payload.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for msg in messages:
                    ...
                    await publish_event("WhatsAppMessage", event)
